...
def val(args, val_loader, model, criterion):
    ...
    for iter, batched_inputs in enumerate(val_loader):
        ...
        # run the mdoel
        output= model(input_var, depth_var, test=False)
...
def train(args, train_loader, model, criterion, optimizer, epoch, max_batches, cur_iter=0, lr_factor=1.):
    # switch to train mode # max_batches=220, cur_iter=220 * epoch
    model.train()
    ...
    for iter, batched_inputs in enumerate(train_loader):
        ...
        # gt_sim = ssim(depth_output, target_var)
        # depth_sim = ssim(depth_output, depth_var)
        ...
        if iter % 5 == 0:
            print('\riteration: [%d/%d] lr: %.7f loss: %.3f time:%.3f' % (
            iter + cur_iter, max_batches * args.max_epochs, lr, loss.data.item(), time_taken),
                  end='')
    ...
    F_beta, MAE, _, _ = salEvalTrain.getMetric()
...
def trainValidateSegmentation(args):
    model = net.AirSOD(args=args,pretrained=True)
    ...
    # whether use multi-scale training, default False
    if args.ms:
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        cur_iter += len(trainLoader)  # max_batches=220, cur_iter=220 * epoch
        ...
        torch.save({
            ...
            'F_val': F_beta_val,
            'lr': lr
        }, args.savedir + 'checkpoint.pth.tar')  # 'epoch': epoch + 1,

        # save the model also
        model_file_name = args.savedir + '/model_' + str(epoch) + '.pth'  # str(epoch + 1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_workers', type=int, default=0, help='No. of parallel threads')  # default=4
